...
sComando cargarComando(string linea){
    ...
    if(tipo == 0){
        ...
        cout<<"Error en el tipo de comando"<<endl;
    }
    return comando;
}
...
void imprimirElementos(list <sElemento> elementos){
    for(auto it=elementos.begin();it!=elementos.end();it++){
        ...
        cout<<"Tamaño: " <<it->tamaño << " "<<it->unidadMedida<<endl;
...
list<char*> entrada;
    ...
    if(strcmp(*it,"help")==0){
        ...
        if(i < 4){
            ...
            cin >> movimiento.tipoMovimiento;
            cout << "Ingrese la magnitud: ";
            cin >> movimiento.magnitud;
            cout << "Ingrese la unidad de medida: ";
            cin >> movimiento.unidadMedida;
            comando.movimiento = movimiento;
        ...
        if(i < 4){
            ...
            sComando::sAnalisis analisis;
            cout << "Ingrese el tipo de movimiento: ";
            cin >> analisis.tipoAnalisis;
            cout << "Ingrese la magnitud: ";
            cin >> analisis.objeto;
            cout << "Ingrese la unidad de medida: ";
            cin >> analisis.comentario;
            comando.analisis = analisis;
            Curiosity.agregarComando(comando);
            return;
        }    
    ...
    //simular_comandos 1 1
        ...
        if(i < 3){
            ...
            cout<<"Descripción de la función:\nEl comando debe utilizar la información de puntos de interés almacenada en memoria"
            <<"para ubicarlos en una estructura de datos jerárquica adecuada, que permita luego realizar consultas"
            <<"geográficas sobre estos elementos. Si alguno de los elementos no puede agregarse adecuadamente, debe"
            <<"generarse un mensaje de error, pero deben terminarse de procesar todos los elementos almacenados en"
            <<"memoria\n";
