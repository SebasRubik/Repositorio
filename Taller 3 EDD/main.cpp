...
int main() {
    ...
    vector<sInfoEntrada> infoAgregacion = infoEntradaAgregacion("entrada.txt");
    vector<sInfoEntrada> infoEliminacion = infoEntradaEliminacion("entrada.txt");
    int i=0;
    for(auto agregacion: infoAgregacion){
        tree.root = tree.insert(tree.root, agregacion.dato);
        i++;
    }

    for(auto eliminacion: infoEliminacion){
        tree.root = tree.erase(tree.root, eliminacion.dato);
        i--;
    }


    vector<int> v = inorder(tree.root);

    int mediana = median(v.size());
    cout << v[mediana];
    return 0;
}
